...
\osc{I would still put a comma in the math above, and then start with a lowercase ``where''}
...
\osc{Don't use $>=$, use $\geq$, or $\leq$, respectively. Also, $\rank$ not $rank$.}
...
\osc{Don't use "double quotes", use \LaTeX{}-like ``double quotes''.}
